...
const PersonalDetailsCRUD: React.FC = () => {
    ...
    const updateDetail = (detail: PersonalDetail) => {
        axios.put(`http://localhost:8080/api/personaldetails/update/${detail.id}`, detail,
        {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + localStorage.getItem('token'),
    ...
    const deleteDetail = (detail: PersonalDetail) => {
        axios.delete(`http://localhost:8080/api/personaldetails/delete/${detail.id}`,
        {
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('token'),
            },
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {details.map(detail => (
                    <div key={detail.id} className="p-4 bg-white rounded shadow-md">
                        <p className="text-gray-700 mb-2"><strong>LinkedIn:</strong> {detail.linkedin}</p>
                        <p className="text-gray-700 mb-2"><strong>Social Media:</strong> {detail.socialMedia}</p>
                        <p className="text-gray-700 mb-2"><strong>Website:</strong> {detail.website}</p>
